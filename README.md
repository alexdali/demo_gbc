...
2. Заполнить значения для `RetailCRM`, `Supabase`, `Telegram`
   и basic auth для `/dashboard`:

```bash
DASHBOARD_BASIC_AUTH_USERNAME=dashboard-admin
DASHBOARD_BASIC_AUTH_PASSWORD=strong-basic-auth-password
```
...
4. На `/dashboard` есть кнопка ручной синхронизации, доступная только после basic auth
